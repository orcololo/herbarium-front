...
export default function NewSessionPage() {
  ...
  const [form, setForm] = useState({ tripName: '', startDate: '', endDate: '', location: '', shareCode: '', notes: '' })
  ...
  return (
    <div className="max-w-2xl mx-auto animate-fade-in">
      <div className="mb-8">
        <Link href="/sessions" className="inline-flex items-center gap-2 text-sm font-medium text-[#6D4C41] hover:text-[#1C1B1F] transition-colors mb-4">
        ...
        <div className="bg-white rounded-[20px] shadow-[0_2px_8px_rgba(0,0,0,0.08)] p-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              />
              {errors.tripName && <p className="text-[#E53935] text-xs mt-1">{errors.tripName}</p>}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={e => { setForm(f => ({ ...f, startDate: e.target.value })); setErrors(e => ({ ...e, startDate: '' })) }}
                  className={clsx(
                    "w-full px-4 py-2.5 rounded-[12px] bg-[#F5F5F5] text-sm text-[#1C1B1F] border-2 border-transparent focus:border-[#3D7A52] outline-none transition-colors",
            ...
            {teamMembers.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {teamMembers.map((member, idx) => (
                  <div key={idx} className="flex items-center gap-1.5 bg-[#E8F5E9] text-[#2D5F3F] px-3 py-1.5 rounded-full text-sm font-medium">
                    {member}
                    <button type="button" onClick={() => removeMember(idx)} className="hover:text-[#1C1B1F] transition-colors">
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M18 6L6 18M6 6L18 18" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                    </button>
                  </div>
                ))}
              </div>
