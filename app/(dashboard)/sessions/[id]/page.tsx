...
export default function SessionDetailPage() {
  ...
  const startDateStr = new Date(session.startDate).toLocaleDateString("pt-BR", {
    day: "numeric",
    month: "short",
    year: "numeric",
  });
  const endDateStr = session.endDate
    ? new Date(session.endDate).toLocaleDateString("pt-BR", {
        day: "numeric",
        month: "short",
        year: "numeric",
      })
    ...
    <div className="max-w-4xl mx-auto animate-fade-in pb-12">
        ...
        <div className="absolute -bottom-10 left-8 right-8 bg-white rounded-[20px] shadow-[0_4px_16px_rgba(0,0,0,0.12)] p-6 border border-[#EEEEEE] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-[22px] font-bold text-[#1C1B1F] tracking-tight">
                {session.tripName}
              </h1>
              {!editing && (
                <button
      ...
      {editing && (
        <div className="sticky top-0 z-10 bg-white/95 backdrop-blur-sm border-b border-[#EEEEEE] px-6 py-3 flex justify-end gap-3 -mx-6 mb-6">
          {saveError && (
            <span className="text-[#C62828] text-sm font-medium self-center mr-auto">
              {saveError}
            </span>
          )}
          <button
          ...
          {editing ? (
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-sm text-[#1C1B1F] font-medium px-4 py-2.5 bg-[#F5F5F5] rounded-xl opacity-70">
                  {session.shareCode || "Nenhum"}
                </p>
              </div>
            </div>
          ) : (
            ...
            {editing ? (
              <div className="space-y-4">
                <div className="flex flex-wrap gap-2">
                  {formData.teamMembers.map((member, index) => (
                    <span
                      key={index}
                      className="flex items-center gap-1 text-xs pl-3.5 pr-2 py-1.5 rounded-full bg-[#E8F5E9] text-[#2D5F3F] font-medium border border-[#C8E6C9]"
                    >
                      {member}
                      <button
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="px-5 py-2 rounded-xl bg-[#3D7A52] text-white text-xs font-medium hover:bg-[#2D5F3F] transition-colors"
                  >
                    Adicionar
                  </button>
                </div>
              ...
              <div className="flex flex-wrap gap-2">
                {session.teamMembers.map((member) => (
                  <span
                    key={member}
                    className="text-xs px-3.5 py-1.5 rounded-full bg-[#E8F5E9] text-[#2D5F3F] font-medium border border-[#C8E6C9]"
                  >
                    {member}
                  </span>
                ))}
