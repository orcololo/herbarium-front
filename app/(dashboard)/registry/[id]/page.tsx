...
export default function RegistryDetailPage() {
  ...
  async function handleDelete() {
    if (!confirm('Arquivar este espécime? Ele será marcado como inativo.')) return
  ...
  if (error && !editing && !item) {
    return (
      <div className="max-w-4xl mx-auto text-center py-20">
        ...
        <p className="text-lg font-medium text-[#1C1B1F]">{error ?? 'Espécime não encontrado'}</p>
  ...
  const speciesName = typeof item.species === 'object' ? item.species?.scientificName : 'Espécie desconhecida'
...
373	            </button>
      ...
      {editing && (
        <div className="sticky top-4 z-10 mb-6 bg-white/80 backdrop-blur-md p-4 rounded-[20px] shadow-[0_4px_16px_rgba(0,0,0,0.08)] border border-[#EEEEEE] flex items-center justify-between">
          <span className="text-sm font-bold text-[#1C1B1F]">Editando Espécime</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white rounded-[20px] shadow-[0_2px_8px_rgba(0,0,0,0.08)] p-6 border border-transparent hover:border-[#EEEEEE] transition-colors">
          ...
          <div className="space-y-1">
            <FieldOrInput editing={editing} type="date" label="Data da Coleta" value={editing ? formData.dateCollected : item.dateCollected} onChange={v => handleFieldChange('dateCollected', v)} />
            ...
            <FieldOrInput editing={editing} label="Localidade" value={editing ? formData.locality : item.locality} onChange={v => handleFieldChange('locality', v)} />
            <FieldOrInput editing={editing} label="Município" value={editing ? formData.municipality : item.municipality} onChange={v => handleFieldChange('municipality', v)} />
            <FieldOrInput editing={editing} label="Estado" value={editing ? formData.state : item.state} onChange={v => handleFieldChange('state', v)} />
            <FieldOrInput editing={editing} label="País" value={editing ? formData.country : item.country} onChange={v => handleFieldChange('country', v)} />
        ...
        <div className="bg-white rounded-[20px] shadow-[0_2px_8px_rgba(0,0,0,0.08)] p-6 border border-transparent hover:border-[#EEEEEE] transition-colors">
          <div className="flex items-center gap-2 mb-5">
            ...
            <h2 className="text-[11px] font-bold text-[#3D7A52] uppercase tracking-widest">Dados de Observação</h2>
          </div>
          <div className="space-y-1">
            <FieldOrInput editing={editing} label="Estado Fenológico" value={editing ? formData.phenologicalState : item.phenologicalState} onChange={v => handleFieldChange('phenologicalState', v)} />
            <FieldOrInput editing={editing} label="Fenologia (Fournier)" value={editing ? formData.phenologyFournier : item.phenologyFournier} onChange={v => handleFieldChange('phenologyFournier', v)} />
            <FieldOrInput editing={editing} label="Método de Coleta" value={editing ? formData.collectionMethod : item.collectionMethod} onChange={v => handleFieldChange('collectionMethod', v)} />
            <FieldOrInput editing={editing} label="Condição Climática" value={editing ? formData.weatherCondition : item.weatherCondition} onChange={v => handleFieldChange('weatherCondition', v)} />
            <FieldOrInput editing={editing} label="Notas do Clima" value={editing ? formData.weatherNotes : item.weatherNotes} onChange={v => handleFieldChange('weatherNotes', v)} />
            <FieldOrInput editing={editing} label="Fase da Lua" value={editing ? formData.moonPhase : item.moonPhase} onChange={v => handleFieldChange('moonPhase', v)} />
            <FieldOrInput editing={editing} type="number" label="Velocidade do Vento (km/h)" value={editing ? formData.windSpeed : item.windSpeed} onChange={v => handleFieldChange('windSpeed', v)} />
            <FieldOrInput editing={editing} label="Táxons Associados" value={editing ? formData.associatedTaxa : item.associatedTaxa} onChange={v => handleFieldChange('associatedTaxa', v)} />
        ...
        {(editing || hasMorphology) && (
          <div className="bg-white rounded-[20px] shadow-[0_2px_8px_rgba(0,0,0,0.08)] p-6 border border-transparent hover:border-[#EEEEEE] transition-colors">
            <div className="flex items-center gap-2 mb-5">
              ...
              <h2 className="text-[11px] font-bold text-[#3D7A52] uppercase tracking-widest">Morfologia</h2>
            </div>

            <FieldOrInput editing={editing} label="Raiz" value={editing ? formData.morphology?.root : item.morphology?.root} onChange={v => setFormData(prev => ({ ...prev, morphology: { ...(prev.morphology ?? {}), root: v as string | undefined } }))} />

            <MorphSubSection title="Caule">
              <FieldOrInput editing={editing} label="Tipo" value={morphSrc?.stem?.type} onChange={v => setStemField('type', v as string | undefined)} />
              <FieldOrInput editing={editing} label="Cor" value={morphSrc?.stem?.color} onChange={v => setStemField('color', v as string | undefined)} />
              <FieldOrInput editing={editing} label="Tamanho" value={morphSrc?.stem?.size} onChange={v => setStemField('size', v as string | undefined)} />
              <FieldOrInput editing={editing} label="Circunferência" value={morphSrc?.stem?.circumference} onChange={v => setStemField('circumference', v as string | undefined)} />
              <FieldOrInput editing={editing} label="Seiva" value={morphSrc?.stem?.sap} onChange={v => setStemField('sap', v as string | undefined)} />
            </MorphSubSection>
            ...
            <MorphSubSection title="Flor">
              <FieldOrInput editing={editing} label="Inflorescência" value={morphSrc?.flower?.inflorescence} onChange={v => setFlowerField('inflorescence', v as string | undefined)} />
              <FieldOrInput editing={editing} label="Cor" value={morphSrc?.flower?.color} onChange={v => setFlowerField('color', v as string | undefined)} />
              <FieldOrInput editing={editing} label="Tamanho" value={morphSrc?.flower?.size} onChange={v => setFlowerField('size', v as string | undefined)} />
            </MorphSubSection>

            <MorphSubSection title="Fruto">
              <FieldOrInput editing={editing} label="Cor" value={morphSrc?.fruit?.color} onChange={v => setFruitField('color', v as string | undefined)} />
              <FieldOrInput editing={editing} label="Formato" value={morphSrc?.fruit?.format} onChange={v => setFruitField('format', v as string | undefined)} />
              <FieldOrInput editing={editing} label="Tamanho" value={morphSrc?.fruit?.size} onChange={v => setFruitField('size', v as string | undefined)} />
            </MorphSubSection>
            ...
            >
              {uploading ? (
                ...
                <>
                  ...
                  <span className="text-[10px] font-medium text-[#9E9E9E]">Enviar</span>
                </>
              )}
            </button>
          </div>
