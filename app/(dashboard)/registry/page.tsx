...
export default function RegistryPage() {
  ...
  const syncedCount = items.filter(
    (i) => i.syncMetadata?.syncStatus === "synced",
  ).length;
  const pendingCount = items.filter(
    (i) => i.syncMetadata?.syncStatus === "pending",
  ).length;
    ...
    <div className="animate-fade-in">
      ...
      <div className="mb-6 flex items-center justify-between gap-4">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            type="search"
            placeholder="Buscar por espécie, número de coleta…"
            value={search}
            onChange={(e) => {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {loading
          ...
          : items.map((item) => {
              const cardImage = getRegistryCardImage(item);
              const collectorName = getRegistryCollectorDisplay(item);
              const coCollectors = getRegistryCoCollectorsDisplay(item);
              const commonName = getRegistryCommonName(item);
              const collectionNumber = getRegistryCollectionNumber(item);
              ...
              return (
                ...
                >
                  <div className="bg-white rounded-2xl shadow-[0_2px_8px_rgba(0,0,0,0.08)] overflow-hidden transition-all duration-200 group-hover:-translate-y-1 group-hover:shadow-[0_8px_24px_rgba(0,0,0,0.12)] border border-transparent group-hover:border-[#EEEEEE]">
                    <div
                      className={clsx(
                        "h-40 flex items-center justify-center relative overflow-hidden",
                        cardImage
                          ? "bg-[#F5F5F5]"
                          : `bg-linear-to-br ${getGradient(item.id)}`,
                      )}
                    >
                      {cardImage ? (
                        <img
                      ...
                      <div className="absolute top-3 right-3">
                        <span
                          className={clsx(
                            "text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full shadow-sm backdrop-blur-sm",
                            STATUS_COLORS[
                    ...
                    <div className="p-5">
                      <p className="text-sm font-semibold text-[#1C1B1F] italic truncate">
                        {getRegistrySpeciesName(item)}
                      </p>
                      {commonName && (
                        <p className="text-xs text-[#6D4C41] truncate mt-0.5">
                          {commonName}
                        </p>
                      )}
                      <div className="mt-3 space-y-2">
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-[10px] font-bold uppercase tracking-wider text-[#9E9E9E]">
                            Número de Coleta
                          </span>
                          <span className="text-[11px] text-[#49454F] font-mono bg-[#F5F5F5] px-2 py-0.5 rounded-md truncate max-w-[55%]">
                            {collectionNumber}
                          </span>
                        </div>
                        {(collectorName || item.collectorNumber) && (
                          <div className="flex items-center justify-between gap-2 text-xs text-[#6D4C41]">
                            <span className="truncate">
                              {collectorName ?? "Coletor não informado"}
                            </span>
                            {item.collectorNumber && (
                              <span className="shrink-0 font-mono text-[#9E9E9E]">
                                #{item.collectorNumber}
                              </span>
                            )}
                          </div>
                        ...
                        {coCollectors && (
                          <p className="text-[11px] text-[#6D4C41] truncate">
                            Co-coletores: {coCollectors}
                          </p>
                        )}
                      </div>
                    </div>
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center bg-white rounded-3xl border border-[#EEEEEE] shadow-sm mt-6">
          <div className="relative mb-6">
            ...
            <svg
              ...
              viewBox="0 0 40 40"
              fill="none"
              className="text-[#3D7A52] absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
            >
              <path
                d="M20 5C20 5 8 11 8 22C8 28.627 13.373 34 20 34C26.627 34 32 28.627 32 22C32 11 20 5 20 5Z"
                stroke="currentColor"
                strokeWidth="2"
              />
              <path
                d="M20 5C20 5 20 18 14 22"
                stroke="currentColor"
                strokeWidth="2"
                strokeLinecap="round"
              />
          ...
          <p className="text-xl font-semibold text-[#1C1B1F]">
            Nenhum espécime encontrado
          </p>
          <p className="text-sm text-[#6D4C41] mt-2 max-w-sm">
            Abra o aplicativo Folium para coletar espécimes e sincronizá-los com
            o painel.
          </p>
        </div>
      )}
