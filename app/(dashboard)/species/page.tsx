...
export default function SpeciesPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await api.species.list({
        page,
        limit: LIMIT,
        ...
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-full bg-[#E8F5E9] flex items-center justify-center text-[#3D7A52] shadow-sm">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
              ...
              />
              <path
                d="M12 5C12 5 9 8 9 11M12 5C12 5 15 8 15 11M12 19C12 19 9 16 9 13M12 19C12 19 15 16 15 13M5 12C5 12 8 9 11 9M19 12C19 12 16 9 13 9M5 12C5 12 8 15 11 15M19 12C19 12 16 15 13 15"
                stroke="currentColor"
                strokeWidth="2"
                strokeLinecap="round"
              />
          ...
          : items.map((sp) => (
              <Link key={sp.id} href={`/species/${sp.id}`} className="block">
                <div className="bg-white rounded-[20px] shadow-[0_2px_8px_rgba(0,0,0,0.04)] px-6 py-5 flex items-center gap-5 border border-transparent hover:-translate-y-0.5 hover:shadow-[0_8px_24px_rgba(0,0,0,0.08)] hover:border-[#EEEEEE] transition-all duration-200">
                  <div className="w-10 h-10 rounded-full bg-[#3D7A52] flex items-center justify-center shrink-0 shadow-sm">
                    <span className="text-white font-mono font-bold text-lg">
                      {sp.scientificName.charAt(0).toUpperCase()}
                    </span>
                  </div>

                  <div className="flex-1 min-w-0">
                    <p className="text-base font-semibold text-[#1C1B1F] italic truncate">
                      {sp.scientificName}
                    </p>
                    <div className="flex items-center gap-3 mt-1 flex-wrap">
                      ...
                      {sp.family && (
                        <span className="text-xs font-medium text-[#9E9E9E] bg-[#F5F5F5] px-2 py-0.5 rounded-md">
                          {sp.family}
                        </span>
                      )}
                    </div>
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center bg-white rounded-3xl border border-[#EEEEEE] shadow-sm mt-6">
          <div className="relative mb-6">
            ...
            >
              ...
              />
              <path
                d="M20 6C20 6 14 9 14 14M20 6C20 6 26 9 26 14M20 34C20 34 14 31 14 26M20 34C20 34 26 31 26 26M6 20C6 20 9 14 14 14M34 20C34 20 31 14 26 14M6 20C6 20 9 26 14 26M34 20C34 20 31 26 26 26"
                stroke="currentColor"
                strokeWidth="2"
                strokeLinecap="round"
              />
          ...
          <p className="text-sm text-[#6D4C41] mt-2 max-w-sm">
            As espécies aparecerão aqui automaticamente conforme os espécimes
            forem sincronizados do aplicativo de campo.
          </p>
        </div>
      )}
