...
type BackendIdentified = Record<string, unknown> & {
  id?: unknown;
  _id?: unknown;
};
...
function normalizeEntityId<T>(item: T): T {
  if (item === null || typeof item !== "object") return item;
  const record = item as BackendIdentified;
  if (record.id !== undefined || record._id === undefined) return item;
  const { _id, ...rest } = record;
  return { ...rest, id: String(_id) } as T;
}

function normalizePaginated<T>(
  response: PaginatedResponse<T> | BackendPaginatedResponse<T>,
): PaginatedResponse<T> {
  if ("meta" in response) {
    return {
      data: response.data.map(normalizeEntityId),
      total: response.meta.total,
      page: response.meta.page,
      limit: response.meta.limit,
    };
  }
  return { ...response, data: response.data.map(normalizeEntityId) };
}

function normalizeUpload(
  result: UploadResult | BackendUploadResult,
): UploadResult {
  return {
    key: result.key,
...
function encodeImageKeyPath(key: string): string {
...
async function uploadFile(
  ...
  const envelope = (await res.json()) as
    | { success?: boolean; data?: UploadResult | BackendUploadResult }
    | UploadResult
    | BackendUploadResult;
  const data =
    "success" in envelope && envelope.success ? envelope.data : envelope;
...
export const api = {
  ...
  registry: {
    list: (params?: {
      ...
      if (params?.isDraft !== undefined)
        q.set("isDraft", String(params.isDraft));
      if (params?.sortBy) q.set("sortBy", params.sortBy);
      if (params?.sortOrder) q.set("sortOrder", params.sortOrder);
      return request<
        PaginatedResponse<Registry> | BackendPaginatedResponse<Registry>
      >(`/registries?${q.toString()}`).then(normalizePaginated);
    },
    ...
    update: (id: string, data: UpdateRegistryPayload) =>
      request<Registry>(`/registries/${id}`, {
        method: "PATCH",
        body: JSON.stringify(data),
      }),
    attachImage: (id: string, file: File) => {
      const formData = new FormData();
      formData.append("file", file);
      return request<Registry>(`/registries/${id}/images`, {
        method: "POST",
        body: formData,
      });
    },
    removeImage: (id: string, key: string) =>
      request<Registry>(`/registries/${id}/images/${encodeImageKeyPath(key)}`, {
        method: "DELETE",
      }),
    delete: (id: string) =>
  ...
  sessions: {
    list: (params?: {
      ...
      return request<
        | PaginatedResponse<CollectionSession>
        | BackendPaginatedResponse<CollectionSession>
      >(`/sessions?${q.toString()}`).then(normalizePaginated);
    },
  ...
  species: {
    list: (params?: { page?: number; limit?: number; search?: string }) => {
      ...
      if (params?.search) q.set("search", params.search);
      return request<
        PaginatedResponse<Species> | BackendPaginatedResponse<Species>
      >(`/species?${q.toString()}`).then(normalizePaginated);
    },
    ...
    delete: (id: string) =>
      request<void>(`/species/${id}`, { method: "DELETE" }),
  },
  ...
  users: {
    list: (params?: {
      ...
      return request<PaginatedResponse<User> | BackendPaginatedResponse<User>>(
        `/users?${q.toString()}`,
